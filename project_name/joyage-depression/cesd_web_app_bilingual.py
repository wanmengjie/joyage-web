...
LANGUAGES = {
    ...
    'ko': {
        ...
        'medium_risk_label': '중간 위험',
        'high_risk_label': '높은 위험',
...
def load_models():
    """优化的模型加载器 - 支持多重备选策略"""
    # 改为以脚本所在目录为基准，避免工作目录变化导致找不到模型
    models_dir = Path(__file__).parent / "saved_models"
    
    if not models_dir.exists():
        return None, None, f"❌ Models directory not found: {models_dir} / 模型目录不存在"
        
    # 模型优先级策略
    model_priorities = [
        ("cesd_model_best_*hyperparameter_tuned*.joblib", "🎯 调优模型 / Tuned Model / 조정된 모델"),
        ("cesd_model_best_latest.joblib", "📊 最佳模型(最新) / Best Latest / 최신 최적"),
        ("cesd_model_best_*.joblib", "📊 最佳模型 / Best Model / 최고 모델"), 
        ("cesd_model_*.joblib", "⚙️ 备用模型 / Backup Model / 백업 모델"),
        ("ADABOOST_pipeline.joblib", "🟣 AdaBoost pipeline"),
    ]
    ...
    if model is None:
        # 最后一种兜底：任意 .joblib，取最新
        any_joblibs = sorted(models_dir.glob("*.joblib"), key=lambda p: p.stat().st_mtime, reverse=True)
        if any_joblibs:
            try:
                model = joblib.load(any_joblibs[0])
                model_info = f"Auto-loaded: {any_joblibs[0].name}"
            except Exception:
                pass
    if model is None:
        return None, None, f"❌ No valid model found in {models_dir} / 未找到有效模型"

    # 从 Pipeline 的 pre 中提取真实使用的原始列名（供前端表单与预处理对齐）
    try:
        used_cols = []
            ...
            if hasattr(pre, 'transformers'):
                for _, _, cols_spec in pre.transformers:
                    if cols_spec is not None and cols_spec != 'drop' and cols_spec != []:
                        used_cols.extend(list(cols_spec))
        # 去重并过滤到已知特征
        if used_cols:
            seen = set(); ordered = []
            for c in used_cols:
            ...
            st.session_state['used_features'] = ordered
    except Exception:
        pass
...
def get_feature_groups():
    ...
    used = set(st.session_state.get('used_features') or [])
    if used:
        for k in list(groups.keys()):
            groups[k] = [f for f in groups[k] if f in used]
    return groups

def _create_basic_info_inputs(inputs, lang):
    """创建基本信息输入"""
    # 年龄
    inputs['agey'] = st.sidebar.slider(get_feature_label('agey'), 60, 110, 65)
...
def validate_and_clean_inputs(inputs):
    ...
    # 数值型变量验证
    used = set(st.session_state.get('used_features') or [])
    for var in [v for v in NUMERICAL_VARS if (not used or v in used)]:
        if var in inputs:
            try:
    ...
    # 分类变量验证
    for var in [v for v in CATEGORICAL_VARS if (not used or v in used)]:
        if var in inputs:
...
def preprocess_input(inputs, data_processor):
    ...
    try:
        ...
        # Step 3: 确保特征顺序一致
        used = st.session_state.get('used_features')
        if used:
            all_features = [f for f in (CATEGORICAL_VARS + NUMERICAL_VARS) if f in used]
        else:
...
def get_model_type(_model):
    ...
    if any(tree_name in model_name for tree_name in ['forest', 'tree', 'gradient', 'xgb', 'lgb', 'catboost', 'adaboost']):
        return 'tree'
...
def _unwrap_pipeline_and_transform(model, X):
    """若是 sklearn Pipeline/Calibrated 包装：
    - 取出预处理器 pre，并对 X.transform → Xt
    - 取出底层分类器（cal.estimator 或 clf）作为 SHAP 的目标模型
    返回 (estimator, Xt)
    """
    est = model
    Xt = X
    try:
        # sklearn Pipeline: 期望步骤名 pre / cal / clf
        if hasattr(model, "named_steps"):
            pre = model.named_steps.get("pre")
            if pre is not None:
                Xt = pre.transform(X)
            est = model.named_steps.get("cal") or model.named_steps.get("clf") or model
        # CalibratedClassifierCV: 取其底层 estimator（prefit 情况）
...
def _load_bg_samples_like_input(X: pd.DataFrame) -> np.ndarray:
    """从 saved_models/X_bg_raw.npy 读取背景样本；若无，则用当前 X 复制作为兜底。"""
    try:
        bg_path = Path(__file__).parent / "saved_models" / "X_bg_raw.npy"
        if bg_path.exists():
            bg = np.load(bg_path)
            if bg.ndim == 2 and bg.shape[1] == X.shape[1]:
                return bg
    except Exception:
        pass
    # 兜底：重复当前样本作为背景
    ...
    if arr.ndim == 1:
        arr = arr.reshape(1, -1)
    return np.repeat(arr, repeats=100, axis=0)


def compute_shap_explanation(model, X, feature_names):
    """优化的SHAP计算 - 支持 Pipeline/Calibrated 解包与错误恢复"""
    try:
        # 数据验证
        ...
        # 若为 Pipeline/Calibrated 包装，先解包并做变换
        est, Xt = _unwrap_pipeline_and_transform(model, X)
        # 对于 KernelExplainer，优先用校准器（若存在），更贴近网页显示概率
        cal_or_est = model
        if hasattr(model, "named_steps") and (model.named_steps.get("cal") is not None):
            cal_or_est = model.named_steps["cal"]
        else:
            cal_or_est = est
        # 预处理输出的列名（优先使用 pre.get_feature_names_out）
        feat_names = None
        try:
            if hasattr(model, "named_steps") and "pre" in model.named_steps:
                pre = model.named_steps["pre"]
        ...
        if feat_names is None:
            # 若维度与原始列一致，用原始列名；否则生成占位名
            feat_names = list(X.columns) if len(X.columns) == D else [f"f{i}" for i in range(D)]
        # 存储用于可视化的列名
        st.session_state['feature_names_for_shap'] = feat_names
        model_type = get_model_type(est)
        
        with st.spinner("🔍 Computing SHAP explanations..."):
            # 直接使用 KernelExplainer（去除 TreeExplainer / 通用 Explainer 分支，避免不兼容提示）
            try:
                bg = _load_bg_samples_like_input(pd.DataFrame(Xt_np, columns=feat_names))
                target = cal_or_est
                if hasattr(target, 'predict_proba'):
                    f = lambda data: target.predict_proba(np.asarray(data, dtype=float))[:, 1]
                elif hasattr(target, 'decision_function'):
                    f = lambda data: target.decision_function(np.asarray(data, dtype=float))
                else:
                    f = lambda data: target.predict(np.asarray(data, dtype=float)).astype(float)
                kexp = shap.KernelExplainer(f, np.asarray(bg, dtype=float))
                shap_values = kexp.shap_values(Xt_np[0:1, :], nsamples=300)
                expected_value = getattr(kexp, 'expected_value', 0.0)
                st.info("✅ Using KernelExplainer")
            except Exception as e:
                st.error(f"❌ SHAP KernelExplainer failed: {e}")
                return None, None
...
def create_shap_waterfall_plot(shap_values, expected_value, feature_names, feature_values):
    ...
    try:
        ...
        # 准备数据
        # 去掉管道前缀（如 num__/bin__/mul__）以更贴近输入变量名
        def _clean(n: str) -> str:
            try:
                return n.split("__", 1)[1] if "__" in n else n
            except Exception:
                return n
        top_features = [_clean(feature_names[i]) for i in top_indices]
    ...
    with col2:
        ...
        if st.session_state.get('prediction_done', False):
            ...
            if shap_values is not None:
                feat_names = st.session_state.get('feature_names_for_shap') or list(X.columns)
                fig = create_shap_waterfall_plot(shap_values, expected_value, feat_names, X.iloc[0])
                if fig:
